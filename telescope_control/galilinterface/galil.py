...
class GalilInterface:
	...
	def __init__(self, ip, port = 23, fakeGalil = False, poll = False, resetGalil = False, unsol = True):
		...
		if resetGalil:
			...
			self.resetGalil(download = True)
			
		if poll:
			print "Beginning to poll Galil"
			self.startPolling()
		if unsol:
	...
	def __initUnsolicitedMessageSocket(self):
		...
		print self.recieveOnly(self.unsolCon)
	...
	def pollUnsol(self):
		...
		while self.running:
			...
			try:
				retString.extend(self.unsolCon.recv(1))
			...
			if "\r\n" in retString:                # 
				...
				if logTimeStamps:
					if message.find("Input Timestamp") + 1:
						# for some bizarre reason, the galil returns
						# timestamps with four trailing zeros (e.g. xxx.0000)
						# The timestamps are ALWAYS just an integer
						# Anyways, the python int() function can't handle
						# strings with a decimal, so we split off the 
	...
	def pollUDP(self, udpSock, galilAddrTup):
		...
		while self.running:
			...
			try:
				...
				dr = self.parseDataRecord(dat)
				if dr:
					print dr["I"]["sampleNumber"]
					fp.write("DR Received, %s, %s, %s\n" % (time.time(), dr["I"]["sampleNumber"], time.strftime("Datalog - %Y/%m/%d, %a, %H:%M:%S", time.localtime())))
				else:
					fp.write("Bad DR Received, %s, %s\n" % (time.time(), time.strftime("Datalog - %Y/%m/%d, %a, %H:%M:%S", time.localtime())))
			except socket.timeout:                  # Exit on timeout
				pass
...
if __name__ == "__main__":
	...
	gInt = GalilInterface(ip = "192.168.1.250", fakeGalil=False, poll = True, resetGalil = True, unsol = True)
